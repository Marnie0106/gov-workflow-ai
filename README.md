# 数字政务一网统管轻量化编排系统 (gov-workflow-ai)

基于 React + Express + SQLite + AI 的数字政务工单全流程管理平台，覆盖市民上报→AI分类派单→处置员处理→领导实时看板的完整闭环。

## 功能特性

- **市民端**：手机号验证码登录、实名认证、工单上报、照片上传、留言沟通、服务评价
- **处置员端**：工单接收/处理、状态流转、AI 处置建议、内部协作、留言互动、**修改派单部门（修正AI分类错误）**
